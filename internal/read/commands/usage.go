...
func (u *usager) String() string {
	...
	if u.Flags != nil {
		f := &flags.DebugReadFlags{}
		clientFlags := f.Flags()

		var debugReadFlags, cmdFlags *flag.FlagSet
		u.Flags.VisitAll(func(f *flag.Flag) {
			if contains(clientFlags, f) {
				if debugReadFlags == nil {
					debugReadFlags = flag.NewFlagSet("", flag.ContinueOnError)
				}
				debugReadFlags.Var(f.Value, f.Name, f.Usage)
			} else {
				if cmdFlags == nil {
